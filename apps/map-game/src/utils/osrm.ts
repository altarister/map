import type { LocationPoint } from '../game/core/types';
 ...
 */
export async function fetchRealWorldRoute(waypoints: LocationPoint[]): Promise<RouteGeometry | null> {
  // 최소 2개의 좌표가 필요
  ...
  try {
    ...
    console.warn('[OSRM Route Warning] 외부 통신 실패. 궤적 그리기를 포기하고 null을 반환합니다.', error);
    // 복잡한 폴백 없이 단순 실패 처리
    return null;
  }
}
