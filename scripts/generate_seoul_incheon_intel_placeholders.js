...
async function run() {
  ...
  for (const [guCode, dongList] of Object.entries(byGu)) {
    ...
    // 파일명: {그 구코드}_{도시}_{구이름}.json (소문자 영문으로)
    const guNameEng = guName
      .replace('구', 'gu').replace('군', 'gun')
      .replace(/[가-힣]/g, '');  // 나머지 한글 제거 (영문 없는 경우 코드 사용)
    const fileName  = `${guCode}_${cityName === '서울' ? 'seoul' : 'incheon'}_${guNameEng || guCode}.json`;
