...
- **기능**:
  - **집중 학습**: 사용자가 학습할 시/군(예: 광주시, 성남시)을 N개 선택하여 게임 시작.
  - **데이터 렌더링**: 수도권 지도 중 선택된 지역의 하위(읍/면/동) 경계만 활성화.
  - **퀴즈 생성**: 선택된 지역 내의 행정구역만 랜덤하게 문제로 출제 (타 지역 출제 X).
...
다시 이야기해 줄께 
게임이 시작되기전에 암기해야할 (시군구)를 n개를 선택해 시작 버튼을 클릭하면 그때 지도 레이어가 닫히고 게임이 시작되 상단에는 아까 선택된 시군구의 하위 지역만 노출되 
광주시를 선택하고 들어왔다면 초월읍 곤지암읍.. 이런것들이 나오고 다른 용인시 처인구 는 문제로 출제되지 않아야해 .