...
| 줌 레벨    | 표시 데이터      | 개수  | 색상           |
| ---------- | ---------------- | ----- | -------------- |
| `k < 1.5`  | 시/군/자치구 | 42개  | 단색 (#e0e7ff) |
| `k >= 1.5` | 읍/면/법정동 | 563개 | 해시 기반 컬러 |

**시각적 계층 시스템 (Visual Hierarchy System) 및 렌더링 최적화**

지도의 정보 과부하(Clutter)와 성능 저하(DOM 폭발), 백그라운드 폴리곤과의 이빨 빠짐 현상(Topo-gap)을 방지하기 위해 다음 렌더링 규칙을 강제합니다.

1.  **매크로 뷰 (Macro View)**: "숲을 보는 단계"
    - **조건**: `줌 레벨 < 1.5` ([시/군/자치구 선택 화면])
    - **표시 내용**: 시/군/자치구 명칭 및 폴리곤. 전국을 한 번에 그리지 않고, 타겟 광역 단체(예: 경기도)에 속한 42개 구역만 렌더링함.

2.  **마이크로 뷰 (Micro View)**: "나무를 보는 단계"
    - **조건**: `줌 레벨 ≥ 1.5` ([읍/면/법정동 퀴즈 화면] 진입 시)
    - **표시 내용**: 읍/면/법정동 경계 및 명칭.
    - ⚠️ **안티 Topo-Gap 렌더링 수칙**: 
      - 읍/면/법정동 폴리곤들을 그릴 때는, **바탕에 깔려 있던 상위 시/군/자치구 폴리곤을 배열에서 완전히 제거(Culling)**합니다.
      - 이를 통해 경계선 픽셀 오차로 인한 하얀 실선 스트로크 노출(이빨 빠짐 현상)을 원천 차단합니다.
...
### 4.6 행정구역 드릴다운 UX (Region Select Flow)

**통계청 데이터 기준의 계층 구조를 기반으로 지역 선택 플로우를 제공합니다.**

#### [1. 광역 자치단체 선택 화면] (맵 전역 뷰)
- **UI:** 서울, 인천, 경기 등 큰 덩어리가 전체 지도에 렌더링됨.
- **동작:** 유저가 특정 지역 클릭 시 해당 광역 단체의 하위 화면으로 부드럽게 줌/전환됨. (예: 서울특별시 클릭)

#### [2. 시/군/자치구 선택 화면] 평등(Flat) 선택
- **UI:** 그리드 레이아웃의 시/군/자치구 버튼 리스트 (예: 강남구, 종로구 / 용인시, 광주시 등)
- **렌더링 원칙:** 대도시의 하위 일반구를 평면 노출하지 않고 유저 상식에 맞춰 시 단위(용인시 덩어리)로 묶어 노출함. 강남구든 광주시든 용인시든 경기도를 누르면 경기도에 속한 시/군/자치구가 **동등한 레벨의 조각(Polygon)**으로 쫙 펼쳐집니다. 
- **동작:** 타겟 구역을 선택하면 해당 구역만 지도 화면에 꽉 차게 줌인(Zoom In)됨. (수원, 용인 등 거대 도시는 [3. 대도시 일반구 선택 화면]으로 중간 이동. 가평, 강남구 등 하위 구가 없으면 바로 4단계 진입)

#### [3. 대도시 일반구 선택 화면] (거대 도시 전용 중간 화면)
- **UI:** 용인시 기흥구, 처인구, 수지구 등을 선택하는 징검다리 인터페이스.
- **동작:** 일반구를 선택 시 비로소 4단계 실전 퀴즈로 진입.

#### [4. 읍/면/법정동 퀴즈 화면] 실전 퀴즈 타겟
- **UI:** 유저가 개입하여 고르는 화면이 아님.
- **동작:** 선택한 구/시 내부에 속한 모든 **'읍/면/법정동' (배차 플랫폼 노출 기준)** 폴리곤들이 활성화되며, 시스템이 무작위로 타겟을 지정하여 퀴즈 출제 시작. (명칭 포맷: `강남구 역삼동`, `광주시 초월읍` 등)
...
제시된 **읍/면/법정동**의 위치를 지도에서 클릭 (상위 시/군 단위 통째로 출제 금지)

#### 출제 규칙 (Question Generation Rules)

1.  **범위 제한 (Scope Limitation)**:
    - 게임 시작 전 사용자가 고른 상위 **시/군/자치구/일반구** 내의 하위 지역만 출제 범위에 포함됩니다.
    - 예: 사용자가 **'강남구'**를 선택했다면, 문제 정답은 **'역삼동', '삼성동', '대치동'** 등에서만 나옵니다.
    - 선택하지 않은 타 구(예: 송파구)는 절대 정답으로 나오지 않습니다.

2.  **대상 데이터 (Target Data)**:
    - 실 주행 경로 및 택배 배차 기준지인 **읍/면/법정동** 데이터만을 출제 단위로 사용합니다.
    - (효용성 문제로 **리(里)** 단위 퀴즈는 시스템에서 영구 파기)

3.  **표시 명칭 (Naming Format)**:
    - 항상 **`[상위 시/군/자치구/일반구] [읍/면/법정동]`** 형태로 표시합니다.
    - 예: `광주시 초월읍`, `수원시 장안구 영화동`, `서울 중구 명동`
    - 동명이인 지역(예: 전국 곳곳에 있는 '중앙동'이나 '신사동') 혼동을 방지하고, 명확한 상위 소속을 인지시키기 위함입니다.
...
- **LOD 전환 시간**: 100ms 이하 (시/군/자치구 ↔ 읍/면/법정동)
