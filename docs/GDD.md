...
1.  **매크로 뷰 (Macro View)**: "숲을 보는 단계"
    - **조건**: `줌 레벨 < 1.5` (Level 2 선택 화면)
    - **표시 내용**: 시/군/구(Level 2) 명칭 및 폴리곤. 전국을 한 번에 그리지 않고, 타겟 Level 1(예: 경기도)에 속한 42개 구역만 렌더링함.

2.  **마이크로 뷰 (Micro View)**: "나무를 보는 단계"
    - **조건**: `줌 레벨 ≥ 1.5` (Level 3 지역 코스 진입 시)
    - **표시 내용**: 읍/면/동(Level 3) 경계 및 명칭.
    - ⚠️ **안티 Topo-Gap 렌더링 수칙**: 
      - Level 3(읍면동) 폴리곤들을 그릴 때는, **바탕에 깔려 있던 해당 Level 2(구) 폴리곤을 배열에서 완전히 제거(Culling)**합니다.
...
### 4.6 행정구역 3-Level 드릴다운 UX (Region Select Flow)

**통계청 데이터 기준의 3단계 계층 구조(3-Level Hierarchy)를 기반으로 지역 선택 플로우를 제공합니다.**

#### 1단계 (Level 1): 광역 자치단체 화면 (맵 전역 뷰)
- **UI:** 서울, 인천, 경기 등 큰 덩어리가 전체 지도에 렌더링됨.
- **동작:** 유저가 특정 지역 클릭 시 해당 광역 단체의 하위 화면(Level 2)으로 부드럽게 줌/전환됨. (예: 서울특별시 클릭)

#### 2단계 (Level 2): 기초 자치단체 (시/군/구) 평등(Flat) 선택 화면
- **UI:** 그리드 레이아웃의 시/군/구 버튼 리스트 (예: 강남구, 종로구 / 용인시 기흥구, 광주시 등)
- **Flat 렌더링 레이어 (폴더링 폐지):** '용인시' 처럼 복잡한 가상 묶음을 만들지 않습니다. 강남구든 기흥구든 상관없이 경기도를 누르면 경기도에 속한 42개의 시/군/구가 **동등한 레벨의 버튼**으로 쫙 펼쳐집니다. 유저는 시/구를 가리지 않고 이 화면에서 최종 목적 구역을 바로 고릅니다.
- **동작:** 타겟 구역(Level 2)을 선택하면 해당 구역만 지도 화면에 꽉 차게 줌인(Zoom In)되며 게임으로 바로 진입. 주변 지역은 강하게 **딤드(Dimmed)** 혹은 블러 처리됨.

#### 3단계 (Level 3): 실전 퀴즈 타겟 (읍/면/동)
- **UI:** 유저가 개입하여 고르는 화면이 아님.
- **동작:** 2단계에서 고른 구역(Level 2) 내부에 속한 모든 **'읍/면/동' (Level 3, 배차 플랫폼 노출 기준)** 폴리곤들이 활성화되며, 시스템이 무작위로 타겟을 지정하여 퀴즈 출제 시작. (명칭 포맷: `강남구 역삼동`, `광주시 초월읍` 등)
