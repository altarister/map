...
### 4.6 지역 숙달 세부 선택 (Region Level 1 Drill-down UX)

**기존의 단순 시/군/구 클릭 방식을 고도화하여, 사용자에게 훈련 깊이(Depth)를 선택할 수 있는 3단계 모달(RegionSelectModal) 플로우를 제공합니다.**

#### 1단계: 대분류 선택 (City/County)
- **UI:** 그리드 레이아웃의 시/군/구 버튼 리스트 (예: 광주시, 성남시 분당구, 여주시 등)
- **동작:** 유저가 특정 지역 클릭 시 2단계로 부드럽게 슬라이드 넘김 전환.

#### 2단계: 훈련 난이도(Depth) 선택 (Mode)
- **UI:** 선택한 시/구의 하위 학습 목표를 묻는 2개의 대형 버튼 패널.
  - **[기본 코스: 주요 '동·읍·면' 암기]** -> 곧바로 게임 시작. 해당 시 전역의 읍/면/동 덩어리가 퀴즈로 출제됨. (리 단위의 경계는 눈에 띄지 않게 하나로 병합하여 표시)
  - **[상세 코스: 특정 지역 '리' 암기]** -> 3단계 상세 진입 클릭.
- **동작:** 모드 결정에 따라 바로 게임으로 넘어가거나(기본 코스), 세부 타겟 선택(상세 코스)으로 이동.

#### 3단계: 상세 지역 선택 (Eup/Myeon Target)
- **UI:** '리' 단위로 쪼개서 학습할 타겟 읍/면 리스트 (예: 초월읍, 곤지암읍, 도척면 등)
- **동작:** 타겟 선택 시 해당 읍/면 지역만 지도 화면에 꽉 차게 줌인(Zoom In)되며 게임이 시작됨.
  - 주변 지역(타 읍/면)은 강하게 **딤드(Dimmed)** 혹은 블러 처리됨.
  - 타겟 지역 내부의 '리' 경계선들(예: 쌍동리, 산이리)이 활성화되어 퀴즈로 출제됨.

**[나가기 및 상태 관리]**
선택 모달창은 우상단 `X` 버튼 또는 `esc` 키로 언제든 1단계부터 차례로 빠져나갈 수 있으며, 선택이 완료되면 상태가 `PLAYING` 으로 전환되고 모달이 닫힘.
